    ...
    export default {
        ...
        methods:{
            ...
            clickSavePdf(){
                ...
                Request.post('/api/saveDataToTemplate', this.allData,{responseType: 'blob'} ).then((response) => {
        ...
        mounted(){   //页面初始化方法
            ...
            Request.post('/api/initPageUseTemplate', {templateCode:realCode}, ).then((response) => {


                if(response.data.responseCode===0){
                    this.allData=JSON.parse(response.data.responseBody.tempalte);

                    this.allData.leftCardDataList = this.allData.leftCardDataList.filter(function(x) {
                        if(x.cardId===-1)
                            return  false;
                        else return  x;
                    });
                    this.allData.rightCardDataList = this.allData.rightCardDataList.filter(function(x) {
                        if(x.cardId===-2)
                            return  false;
                        else return  x;
                    });
